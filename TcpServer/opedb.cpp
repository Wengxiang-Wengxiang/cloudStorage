...
void OpeDB::init()
{
    ...
    //此处根据个人数据库位置设置路径
    m_db.setDatabaseName("//home//TcpServer//cloud.db");
    if (m_db.open())
    {
        QSqlQuery query;
...
bool OpeDB::handleRegist(const char *name, const char *pwd)
{
    ...
    QString data = QString("insert into userInfo(name,passwd)values(\'%1\', \'%2\')").arg(name).arg(pwd);
//    qDebug() << data;
    QSqlQuery query;
...
bool OpeDB::handleLogin(const char *name, const char *pwd)
{
    ...
    QString data = QString("select * from userInfo where name = \'%1\' and passwd = \'%2\' and online = 0").arg(name).arg(pwd);
//    qDebug() << data;
    QSqlQuery query;
    query.exec(data);
    if (query.next())
    {
        QString data = QString("update userInfo set online = 1 where name = \'%1\' and passwd = \'%2\'").arg(name).arg(pwd);
//        qDebug() << data;
        QSqlQuery query;
        query.exec(data);
        return true;
...
bool OpeDB::handleCanel(const char *name, const char *pwd)
{
    if (NULL == name || NULL == pwd)
    {
        qDebug() << "name or pwd is NULL";
    ...
    if (query.next())
    {
        QString data = QString("delete from userInfo where name = '%1';").arg(name);
        QSqlQuery query;
        query.exec(data);
        return true;
    }
...
void OpeDB::handleAgreeAddFriend(const char *pername, const char *name)
{
    ...
    QString data = QString("insert into friend(id, friendId) values ((select id from userInfo where name = \'%1\'),(select id from userInfo where name = \'%2\'));").arg(name).arg(pername);
//    qDebug() << data << "\n" << pername << name;
    QSqlQuery query;
    query.exec(data);
}
...
bool OpeDB::handleDelFriend(const char *name, const char *friendName)
{
    ...
    QString data = QString("delete from friend where (id = (select id from userInfo where name = \'%1\') and friendId = (select id from userInfo where name = \'%2\'))"
                           "or (id = (select id from userInfo where name = \'%3\') and friendId = (select id from userInfo where name = \'%4\'));").arg(name).arg(friendName).arg(friendName).arg(name);
    QSqlQuery query;
    query.exec(data);
